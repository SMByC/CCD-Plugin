...
class BuildContractTest(unittest.TestCase):
    ...
    def test_workflows_build_plugin_and_extlibs_artifacts(self):
        ...
        self.assertIn('gh api "repos/$REPOSITORY/releases"', release)
        self.assertIn("--method PATCH", release)
        self.assertIn("-F draft=false", release)
        self.assertNotIn("--draft", release)
        self.assertIn('gh release create "$RELEASE_TAG"', release)
        self.assertIn('gh release upload "$RELEASE_TAG" extlibs.zip --clobber', release)
        self.assertNotIn('gh release edit "$RELEASE_TAG" --draft=false', release)
        release_lookup_end = release.index("qgis-plugin-ci release")
        self.assertLess(release.index('gh release view "$RELEASE_TAG"'), release_lookup_end)
        self.assertLess(release.index('gh api "repos/$REPOSITORY/releases"'), release_lookup_end)
        self.assertLess(release.index("--method PATCH"), release_lookup_end)
        self.assertLess(release.index("-F draft=false"), release_lookup_end)
        self.assertLess(release.index('gh release create "$RELEASE_TAG"'), release_lookup_end)
        self.assertLess(release_lookup_end, release.index("gh release upload"))
